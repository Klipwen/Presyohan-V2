...
class StoreActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun showStoreChoiceDialog() {
        ...
        btnJoin.setOnClickListener {
            startActivity(Intent(this, com.presyohan.presyohanapp.JoinStoreActivity::class.java))
            dialog.dismiss()
        }
                                        ...
                                        .addOnSuccessListener { notifSnapshot ->
                                            ...
                                            if (hasAccepted) {
                                                ...
                                                val notification = hashMapOf(
                                                    ...
                                                    "senderId" to com.google.firebase.auth.FirebaseAuth.getInstance().currentUser?.uid.orEmpty(),
                                                    "storeName" to store.name,
                                                    "role" to role,
                                                    "timestamp" to System.currentTimeMillis(),
                                                    "message" to "${com.google.firebase.auth.FirebaseAuth.getInstance().currentUser?.displayName.orEmpty()} invited you to join their store, ${store.name} as $role."
                                                )
                                                db.collection("users").document(staffUserId)
                                                    .collection("notifications")
                                                    .add(notification)

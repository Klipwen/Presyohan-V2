-- RPC: regenerate_invite_code(p_store_id) -> returns invite_code and created_at
-- SECURITY DEFINER to bypass RLS safely; enforces owner-only via explicit check.
...
BEGIN
  ...
  -- Generate a short unique code (uppercase 8 chars); retry on rare collisions
  FOR i IN 1..5 LOOP
...
IS 'Regenerates and returns invite_code for a store; owner-only; SECURITY DEFINER.';

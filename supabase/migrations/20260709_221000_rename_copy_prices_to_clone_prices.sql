-- Migration: Rename copy_prices to clone_prices

BEGIN;

-- Drop the old copy_prices function
DROP FUNCTION IF EXISTS public.copy_prices(UUID, TEXT, UUID[], BOOLEAN);

-- Define clone_prices with the same logic
CREATE OR REPLACE FUNCTION public.clone_prices(
...
BEGIN
  ...
  IF NOT v_is_owner THEN
    RAISE EXCEPTION 'Only source store owner can clone prices' USING ERRCODE = 'insufficient_privilege';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.clone_prices(UUID, TEXT, UUID[], BOOLEAN) TO authenticated;
